...
let cmplz_categories = ["functional", "preferences", "statistics", "marketing"];
...
window.cmplz_set_cookie = function(name, value, use_prefix) {
    ...
    let secure = ";secure";
    let date = new Date;
    date.setTime(date.getTime() + complianz.cookie_expiry * 24 * 60 * 60 * 1e3);
...
function cmplz_enable_category(category, service) {
    ...
    document.querySelectorAll("[data-category=" + category + "], [data-service=" + service + "]").forEach(obj => {
        ...
        let event = new CustomEvent("cmplz_category_enabled", {
            detail: details
        });
        document.dispatchEvent(event)
    });
    ...
    let event = new CustomEvent("cmplz_enable_category", {
        detail: details
    });
    document.dispatchEvent(event);
...
function cmplz_run_after_all_scripts(category) {
    if (!cmplz_all_scripts_hook_fired && cmplz_array_is_empty(cmplz_waiting_inline_scripts) && cmplz_array_is_empty(cmplz_waiting_scripts)) {
        let event = new CustomEvent("cmplz_run_after_all_scripts", {
            detail: category
        });
        document.dispatchEvent(event);
...
function cmplz_run_tm_event(category) {
    if (cmplz_fired_events.indexOf(category) === -1) {
        ...
        let event = new CustomEvent("cmplz_tag_manager_event", {
            detail: category
        });
        document.dispatchEvent(event)
    }
...
window.conditionally_show_banner = function() {
    ...
    event = new CustomEvent("cmplz_cookie_banner_data", {
        detail: complianz
    });
    document.dispatchEvent(event);
...
window.cmplz_set_banner_status = function(status) {
    ...
    let event = new CustomEvent("cmplz_banner_status", {
        detail: status
    });
    document.dispatchEvent(event);
    cmplz_start_clean()
...
function cmplz_set_service_consent(service, consented) {
    ...
    let event = new CustomEvent("cmplz_status_change_service", {
        detail: details
    });
    document.dispatchEvent(event)
}
...
window.cmplz_set_consent = function(category, value) {
    ...
    details.categories = cmplz_accepted_categories();
    cmplz_all_scripts_hook_fired = false;
    let event = new CustomEvent("cmplz_status_change", {
        detail: details
    });
    document.dispatchEvent(event);
...
window.cmplz_deny_all = function() {
    ...
    cmplz_fire_categories_event();
    cmplz_track_status();
    let event = new CustomEvent("cmplz_revoke", {
        detail: reload
    });
    document.dispatchEvent(event);
...
function cmplz_fire_categories_event() {
    ...
    details.categories = cmplz_accepted_categories();
    details.region = complianz.region;
    let event = new CustomEvent("cmplz_fire_categories", {
        detail: details
    });
    document.dispatchEvent(event)
}
...
function cmplz_track_status(status) {
    ...
    let event = new CustomEvent("cmplz_track_status", {
        detail: status
    });
    document.dispatchEvent(event);
    if (!status) {
...
function cmplz_clean() {
    ...
    setTimeout(function() {
        let consent_categories = ["preferences", "statistics", "marketing"];
...
function cmplz_equals(array_1, array_2) {
    ...
    for (var i = 0, l = array_1.length; i < l; i++) {
        if (array_1[i] instanceof Array && array_2[i] instanceof Array) {
            if (!cmplz_equals(array_1[i], array_2[i])) return false
