...
(function() {
        ...
        cont.find('a').each(function() {
            ...
            if (href && href != '#' && !trx_addons_is_local_link(href)) {
                for (var i = 0; i < args.length; i++) {
                    if ((!args[i].page || window.location.href.indexOf(args[i].page) >= 0) && (!args[i].mask || href.indexOf(args[i].mask) >= 0)) {
    ...
    window.trx_addons_browser_is_iphone = function() {
        return navigator.userAgent.match(/iPhone/i) !== null || navigator.platform.match(/(iPhone)/i) ? true : false;
    };
    window.trx_addons_browser_is_ipad = function() {
        return navigator.userAgent.match(/iPad/i) !== null || navigator.platform.match(/(iPad)/i) ? true : false;
    };
    window.trx_addons_browser_is_ipod = function() {
        return navigator.userAgent.match(/iPod/i) !== null || navigator.platform.match(/(iPod)/i) ? true : false;
    };
    ...
    window.trx_addons_wrap_chars = function(txt, before, after) {
        ...
        for (var i = 0; i < txt.length; i++) {
            ...
            rez += in_tag ? ch : before + (txt.substr(i, 1) == ' ' ? '&nbsp;' : txt.substr(i, 1)) + after;
            if (ch == '>') {
                in_tag = false;
            }
        }
...
(function() {
    ...
    jQuery(document).ready(function() {
        ...
        var $show_on_scroll, $banner_placeholder, $animated_elements, $animated_hover, $video_sticky, $fixed_columns, $stack_sections, $parallax_wrap, $video_tags, $iframe_tags, $video_autoplay, $video_autoplay_yt;
        ...
        function trx_addons_ready_actions(e, container) {
            ...
            if ($video_hover.length > 0) {
                $video_hover.addClass('inited').on('click', function(e) {
                    ...
                    $self.parents('.trx_addons_video_player').eq(0).addClass('video_play').find('.video_embed').html($self.data('video'));
                ...
                $video_controller.addClass('inited').on('click', '.trx_addons_video_list_controller_item > a[href="#"]', function(e) {
                    ...
                    if (!item.hasClass('trx_addons_video_list_controller_item_active') && video && video_wrap.length == 1) {
                        ...
                        video_wrap.fadeTo(300, 0, function() {
                            video_wrap.height(video_wrap.height()).html(video + (title ? title : ''));
            ...
            if (TRX_ADDONS_STORAGE['popup_engine'] == 'pretty') {
                container.find(trx_addons_apply_filters('pretty-init-images', 'a[href$="jpg"]:not(.inited):not([target="_blank"])' + ',a[href$="jpeg"]:not(.inited):not([target="_blank"])' + ',a[href$="png"]:not(.inited):not([target="_blank"])' + ',a[href$="gif"]:not(.inited):not([target="_blank"])')).each(function() {
                    var $self = jQuery(this);
                ...
                container.find(trx_addons_apply_filters('mfp-init-images', 'a[href$="jpg"]:not(.inited):not([target="_blank"])' + ',a[href$="jpeg"]:not(.inited):not([target="_blank"])' + ',a[href$="png"]:not(.inited):not([target="_blank"])' + ',a[href$="gif"]:not(.inited):not([target="_blank"])')).each(function() {
                    var $self = jQuery(this);
                ...
                var images = container.find(trx_addons_apply_filters('mfp-init-images-selector', 'a[rel*="magnific"]' + ':not(.inited)' + ':not(.esgbox)' + ':not(.fancybox)' + ':not([target="_blank"])' + ':not(.prettyphoto)' + ':not([rel*="pretty"])' + ':not([data-rel*="pretty"])' + ':not([data-elementor-open-lightbox="yes"])' + ':not([data-elementor-open-lightbox="default"])')).addClass('inited');
                if (images.length > 0) {
                    setTimeout(function() {
                ...
                window.trx_addons_prepare_popup_content = function(container, autoplay) {
                    ...
                    if (wrapper.data('popup-content') === undefined) {
                        ...
                        if (wrapper.find('audio').length || wrapper.find('video').length || (iframe.length && ((iframe.data('src') && iframe.data('src').search(/(youtu|vimeo|daily|facebook)/i) > 0) || iframe.attr('src').search(/(youtu|vimeo|daily|facebook)/i) > 0))) {
                ...
                YTdeferred.done(function(YT) {
                    $video_autoplay_yt.each(function() {
                        ...
                        if (isInView && !isInit) {
                            ...
                            if (!videoID) return;
                            $frame.append('<span class="video_frame_overlay"></span>' + '<span class="video_frame_controls">' + '<a class="video_frame_control_stop video_frame_link" href="https://youtube.com/watch?v=' + videoID + '" target="_blank"></a>' + '<span class="video_frame_control_volume video_frame_control_volume_mute"></span>' + '<span class="video_frame_control_state video_frame_control_state_play"></span>' + '</span>');
...
(function() {
    ...
    $document.on('action.prepare_popup_elements', function(e, cont, mfp) {
        ...
        items.each(function(idx) {
            ...
            $self.removeClass('animated ' + animation + ' ' + trx_addons_elementor_animate_items_animation_revert_name(animation)).addClass('elementor-invisible');
            if (block) {
    ...
    window.trx_addons_elementor_animate_items = function(force) {
        jQuery('[class*="animation_type_"]:not(.animation_type_block)' + (!force ? ':not(.animated-separate)' : '')).each(function() {
            ...
            sc.removeClass('elementor-invisible animated ' + trx_addons_elementor_animate_items_animation(item_params) + (sc.data('last-animation') ? ' ' + sc.data('last-animation') : '') + (item_speed ? ' ' + item_speed : '')).addClass('animated-separate').data('last-animation', trx_addons_elementor_animate_items_animation(item_params));
            if (!cid) {
    ...
    function trx_addons_elementor_add_bg_text(cont) {
        if (cont.hasClass('elementor-section')) {
            ...
            jQuery((typeof window.elementorFrontend !== 'undefined' && elementorFrontend.isEditMode() ? '.elementor-section.elementor-element-edit-mode' : '.trx_addons_has_bg_text') + ':not(.trx_addons_has_bg_text_inited)').each(function() {
    ...
    function trx_addons_elementor_marquee_bg_text(marquee_wrap, marquee_dir, marquee_speed, marquee_time, start, effect) {
        ...
        if (elements.eq(0).hasClass('trx_addons_showed_on_scroll')) {
            ...
            elements.each(function(idx) {
                TweenMax.to(elements.eq(idx), marquee_time - marquee_time * 0.8 * marquee_speed / 10, {
                    x: mw * marquee_dir,
                    y: 0,
        ...
        self.init = function() {
            ...
            if (settings[0].mouse == 1) {
                ...
                if (settings[0].mouse_handler == 'window') {
                    ...
                    $targetLayers = $target.hasClass('trx_addons_parallax_blocks') ? $target.parent() : $target.parents('.elementor-section').eq(0);
                } else if (settings[0].mouse_handler == 'column') {
                    $targetLayers = $target.hasClass('trx_addons_parallax_blocks') ? $target.parent() : $target.parents('.elementor-column').eq(0);
    ...
    function trx_addons_cpt_layouts_fix_rows(args) {
        ...
        rows.each(function(idx) {
            ...
            if ((force_state == 'off' && row.is(force_row)) || (!force_state && (scroll_offset + rows_offset < offset + delay || h < ph))) {
                ...
                if (!row.hasClass('sc_layouts_row_fixed_on')) {
                    if ((force_state == 'on' && row.is(force_row)) || (!force_state && rows_offset + h < trx_addons_window_height() * 0.33)) {
                        if (!row.hasClass('sc_layouts_row_hide_unfixed')) {
                            ph = h;
    ...
    function trx_addons_init_sliders(e, container) {
        if (window.Swiper && container.find('.sc_slider_controller:not(.inited)').length > 0) {
            container.find('.sc_slider_controller:not(.inited)').each(function() {
                ...
                var controller_html = '';
                jQuery('#' + slider_id + ' .slider_container:not(.slider_controller_container) .swiper-slide').each(function(idx) {
        ...
        window.trx_addons_init_sfmenu = function(selector) {
            jQuery(selector).show().each(function() {
                ...
                function trx_addons_stretch_submenu(menu_item) {
                    var done = false;
                    if (menu_item.length && TRX_ADDONS_STORAGE['menu_stretch'] == 1 && !menu_item.hasClass('trx_addons_no_stretch') && !menu_item.parents('.sc_layouts_menu').hasClass('sc_layouts_menu_dir_vertical') && trx_addons_apply_filters('trx_addons_filter_stretch_menu', menu_item.hasClass('sc_layouts_submenu') || menu_item.parent().attr('class').indexOf('columns-') != -1, menu_item)) {
                        ...
                        if (menu.length == 1) {
                            ...
                            if (content_wrap.length == 1) {
                                ...
                                if (stretch_to == 'window') {
                                    menu_item.data('reset-style', '.elementor-section-boxed > .elementor-container').find('.elementor-section-boxed > .elementor-container').css({
                                        'max-width': 'none'
                                    });
        ...
        jQuery('.sc_layouts_menu:not(.sc_layouts_menu_dir_vertical)').each(function() {
            ...
            function check_menu_wrapper(item) {
                ...
                if (!allow_any_wrapper) {
                    var wrappers_list = trx_addons_apply_filters('trx_addons_filter_menu_collapse_wrapper_classes', ['sc_layouts_column', 'wpb_wrapper', 'elementor-widget-wrap', 'wp-block-column', 'kt-inside-inner-col']);
    ...
    function trx_addons_search_close(search_wrap) {
        if (search_wrap.hasClass('search_style_fullscreen')) {
            ...
            search_wrap.fadeOut(500, function() {
                search_wrap.find('.search_results').hide().end().removeClass('search_opened').fadeIn();
            });
        } else {
